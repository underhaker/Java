...
public interface LinkedListInterface {
    ...
    // Removes the first node with element matching the specified value, return
    // true if there's such and false if not.
    ...
    // Checks if both lists contain the same elements in the same order and have
    // the same size.
    public boolean equals(LinkedList otherList);

    // Returns new list with all unique elements of the original list in the
    // same order, in that they're found.
    public LinkedList removeDuplicates();

    // Inserts coppies of all elements of @otherList that are between @start and
    // @end including @start
    public void splice(LinkedList otherList, Node start, Node end);

    // Returns a new list with all elements on the right of @node including
    // @node and removes them from the original list
    // Òhe rest (to the left of @node) should remain in the current list.
